...
describe('MockService.attachToPage', () => {
  test('registers a route and uses handleRoute, continuing when not handled', async () => {
    ...
    await (registeredHandler as any)(fakeRoute);
  ...
  test('aborts when handleRoute and continue both fail', async () => {
    ...
    let registeredHandler: ((route: any) => Promise<void>) | null = null;
    const fakePage = {
      route: async (_pattern: string, handler: (route: any) => Promise<void>) => {
        registeredHandler = handler;
      },
...
test('In record/auto mode, handleRoute rethrows when route.fetch fails', async () => {
  const service = new MockService({
    recordMode: 'auto',
    mockDir: 'unused',
  } as AtticusOptions);
...
test('in replay mode, missing mock returns 500 and does not call fetch', async () => {
  const service = new MockService({
    recordMode: 'replay',
    mockDir: 'unused'
  } as AtticusOptions);
...
test('In auto mode, missing mock triggers live fetch and recording', async () => {
  const service = new MockService({
    recordMode: 'auto',
    mockDir: 'unused',
  } as AtticusOptions);
  ...
  const route = createFakeRoute({
    // fake real network response for fetch()
    fetch: async () => ({
      status: () => 201,
      headers: () => ({ 'content-type': 'application/json' }),
      text: async () => '{"ok":true}',
...
describe('MockService helpers', () => {
  ...
  test('approve logs and does not throw', () => {
    const service = new MockService({
      recordMode: 'auto',
      mockDir: 'unused',
    } as AtticusOptions);
